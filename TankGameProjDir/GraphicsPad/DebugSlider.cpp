...
DebugSlider::DebugSlider(float value, float min, float max, bool textOnLeft, float granularity, const char* name)
{
	QLayout* layout;
	this->min = min;
	this->max = max;
	...
	layout->addWidget(text = new QLabel);
	text->setAlignment(Qt::AlignCenter);
	QString zoomText = QString(name);
	text->setText(zoomText);
